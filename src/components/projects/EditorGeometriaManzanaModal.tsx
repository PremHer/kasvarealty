...
interface EditorGeometriaManzanaModalProps {
  isOpen: boolean
  onClose: () => void
  proyectoId: string
  manzanaId: string
...
export default function EditorGeometriaManzanaModal({
  isOpen,
  onClose,
  proyectoId,
  manzanaId,
  manzana,
  onGeometriaSaved
}: EditorGeometriaManzanaModalProps) {
  ...
  useEffect(() => {
    if (manzana?.geometria) {
      // Parsear WKT existente
      try {
        const wkt = manzana.geometria
        setWktInput(wkt)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/proyectos/${proyectoId}/manzanas/${manzanaId}/geometria`, {
        method: 'PUT',
        headers: {
      ...
      toast({
        title: 'Éxito',
        description: 'Geometría de la manzana guardada correctamente'
      })
      ...
      toast({
        title: 'Error',
        description: 'No se pudo guardar la geometría de la manzana',
        variant: 'destructive'
      })
    } finally {
      setIsLoading(false)
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Editor de Geometría de Manzana
            {manzana?.nombre && (
              <span className="text-sm font-normal text-gray-600">- {manzana.nombre}</span>
            )}
          </DialogTitle>
          <DialogDescription>
            Define la geometría de la manzana haciendo clic en el mapa o ingresando coordenadas WKT.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-green-50 p-3 rounded-lg">
            <h4 className="font-medium text-green-900 mb-2">Instrucciones:</h4>
            <ul className="text-sm text-green-800 space-y-1">
              <li>• Haz clic en el mapa para agregar puntos al polígono de la manzana</li>
