...
interface ProjectContratosProps {
  proyectoId: string
}

export default function ProjectContratos({ proyectoId }: ProjectContratosProps) {
  const { data: session } = useSession()
  const [search, setSearch] = useState('')
  const [estadoFilter, setEstadoFilter] = useState<'TODOS' | 'GENERADO' | 'NO_GENERADO'>('TODOS')

  const { data: contratos = [], isLoading } = useQuery({
    queryKey: ["contratos", proyectoId],
    queryFn: async () => {
      const response = await fetch(`/api/contratos?proyectoId=${proyectoId}`)
      if (!response.ok) {
  ...
  const getPrecio = (contrato: Contrato) => {
    if (contrato.ventaLote) {
      return contrato.ventaLote.precioVenta
    }
    if (contrato.ventaUnidadCementerio) {
      return contrato.ventaUnidadCementerio.precioVenta
    }
    return 0
  }
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Fecha del Contrato</TableHead>
              ...
              {filteredContratos.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-8 text-gray-500">
                    No se encontraron contratos para este proyecto
                  </TableCell>
                </TableRow>
              ) : (
